...
struct SparseMatrix
{
   ...
   bool check_symmetry()
   {
	 for(unsigned int n=0; n<index.size(); ++n){
		 for(unsigned int k=0; k<index[n].size(); ++k){
			if(index[n][k]!=n){
				unsigned int M = index[n][k];
				for(unsigned int j=0; j<index[M].size();++j){
					if(index[M][j] == n){
						if(value[n][k] != value[M][j]){
							printf("Error! matrix not symmetric with n = %u, k = %u, M = %u, j = %u\n",
									n, k, M, j);
							printf("value[n][k] = %f value[M][j] = %f \n", value[n][k], value[M][j]);
							exit(1);
						}
					}
				}
   ...
   bool check_diagnal(unsigned int &N)
  {
 	for(unsigned int n=0; n<index.size(); ++n){
	 for(unsigned int k=0; k<index[n].size(); ++k){
		if(index[n][k]==n){
			if(value[n][k] < 1.e-4){
				printf("Error! matrix diagnal is too small n = %u, k = %u\n",
						n, k);
				printf("value[n][k] = %f \n", value[n][k]);
				N = n;
				return false;
