...
public:
	...
	void GenerateSurface(const RLELevelSet *rle,
	...
	T SurfaceArea(const T* val, const Point* p, T isolevel, float h);
	...
	POINT3DID CalculateIntersection(unsigned int nX, unsigned int nY, unsigned int nZ, unsigned int nEdgeNo, const RLELevelSet *rle);
